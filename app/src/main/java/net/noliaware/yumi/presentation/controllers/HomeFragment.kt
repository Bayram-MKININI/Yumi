...
import net.noliaware.yumi.presentation.views.HomeMenuView
import net.noliaware.yumi.presentation.views.HomeView
    ...
    private fun displayCategoriesFragment() {
        childFragmentManager.beginTransaction().run {
            replace(
                ...
                CategoriesFragment().withArgs(CATEGORIES_DATA to viewModel.connectData?.categories)
            )
            commitAllowingStateLoss()
